...
GitHub Actions의 `잡코리아 채용공고 수집`은 매일 한국시간 오전 9시에 자동 실행됩니다. 설정된 개발자 관련 검색어 전체를 키워드별 5페이지까지 수집하고, 상세 공고를 최대 500건 분석해 매칭 점수순으로 공개합니다. 요청 사이에는 2초 간격을 두며 접근 제한을 우회하지 않습니다. 수동 실행도 지원합니다.

자동 실행은 이전 SQLite DB를 Actions 캐시에서 복원해 결과를 누적하고, 수집 성공 후 `frontend/services/demo-data.ts`가 변경되면 봇 커밋으로 `main`에 반영합니다. 이 커밋이 GitHub Pages 배포를 자동으로 시작합니다.
